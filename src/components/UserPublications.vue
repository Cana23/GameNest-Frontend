        ...
        <div class="flex justify-between items-start">
          ...
          <div class="flex gap-4 items-center">
            ...
            <div class="flex flex-col">
              <router-link
                :to="{ name: 'Perfil', params: { id: publication.userId } }"
                class="text-gray-600 hover:text-purple-600"
              >
                {{ publication.userName || "Usuario desconocido" }}
              </router-link>
              <p class="text-gray-600 text-sm">
                Publicado el {{ formatDate(publication.publicationDate) }}
              </p>
          ...
          <div v-if="isOwner(publication.userId)" class="relative">
            <button
              @click.stop="toggleOptions(publication.id)"
              class="p-1 rounded-full hover:bg-[#4c1d95]/60 transition-colors"
            >
            ...
            <!-- Menú desplegable -->
            <div
              v-if="activeOptions === publication.id"
            ...
            >
              ...
              <button
                v-if="!publication.isHidden"
                @click="
                  hidePublication(publication.id);
                  closeOptions();
                "
                class="flex items-center w-full text-left px-4 py-2 text-sm text-white hover:bg-[#4c1d95]"
              >
              ...
              <button
                v-else
                @click="
                  unhidePublication(publication.id);
                  closeOptions();
                "
                class="flex items-center w-full text-left px-4 py-2 text-sm text-white hover:bg-[#4c1d95]"
              >
        ...
        <div class="w-full px-6 flex flex-col gap-4">
            ...
            <div class="flex gap-5">
              <div class="flex gap-2">
                <i
                  :class="[
                    'pi',
                    publication.hasLiked ? 'pi-heart-fill' : 'pi-heart',
                    'text-violet-500',
                    'cursor-pointer',
                  ]"
              ...
              <div class="flex gap-2">
                <i
                  class="pi pi-comments text-violet-500 cursor-pointer"
                  @click="toggleComments(publication)"
                ></i>
                <p class="text-gray-600 text-sm">
                  {{ publication.comments?.length || 0 }} comentarios
                </p>
              </div>
            </div>
          </div>
          ...
          <div v-if="publication.showComments" class="mt-4">
            <ul class="space-y-2">
              ...
              >
                <p class="text-sm text-white">
                  <strong class="text-[#a78bfa]">{{ comment.nombreUsuario }}:</strong>
                  {{ comment.contenido }}
                </p>
                <p class="text-xs text-[#cbd5e0] mt-1">
                  Publicado el: {{ formatDate(comment.fechaComentario) }}
                </p>
              </li>
            </ul>
            ...
            <form @submit.prevent="addComment(publication)" class="mt-4">
              ...
              <button
                type="submit"
                class="mt-2 bg-gradient-to-r from-[#7c3aed] to-[#a78bfa] text-white px-4 py-2 rounded-lg hover:opacity-90 transition-opacity"
              >
                Agregar comentario
              </button>
            </form>
      ...
      <form @submit.prevent="submitEditForm" novalidate class="w-full flex flex-col gap-5">
          ...
          <FloatLabel variant="on" class="bg-white">
            <InputText
              ...
              :class="{
                ...
                'border-red-500': editErrors.title,
              }"
            />
        ...
        <div class="relative">
          <FloatLabel variant="on" class="bg-white">
            <Textarea
              ...
              optional()
              :class="{
                'w-full text-gray-600 border-1 border-gray-300 rounded-2xl py-3 px-6': true,
                'border-red-500': editErrors.content,
              }"
              rows="5"
              cols="30"
            />
            <label for="editContent" class="bg-white"
              >Comparte tu experiencia y conocimientos para que más personas te conozcan</label
            >
          </FloatLabel>
          <span
        ...
        <div class="flex justify-end gap-4">
          <button
            ...
            class="py-2 px-4 bg-red-500 text-white rounded-xl hover:bg-red-700 cursor-pointer"
          >
            Cancelar
          </button>
          <button
            type="submit"
            class="py-2 px-4 text-white rounded-xl bg-purple-500 hover:bg-purple-600 cursor-pointer"
          >
            Guardar cambios
          </button>
        </div>
...
const [editContentField, editContentAttrs] = defineEditField("content", {
  initialValue: editContent,
});
const [editImageUrlField, editImageUrlAttrs] = defineEditField("imageUrl", {
...
const submitEditForm = async () => {
  ...
  try {
    ...
    toast.add({
      ...
      detail: "La publicación se actualizó correctamente",
    });
    ...
    toast.add({
      ...
      detail: "Hubo un problema al guardar los cambios",
    });
  }
};
